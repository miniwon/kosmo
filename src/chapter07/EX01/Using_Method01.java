...
class B {
	//1. 리턴 타입이 없고(void), 입력매개변수가 없는 메서드
	void print() {
		System.out.println("안녕");
	}
	
	//2. 리턴 타입 int, 입력매개변수 없는 메서드
	int data() {	
		return 3;		//3은 반드시 정수여야 한다
	}
	
	//3. 리턴 타입 double, 입력매개변수가 2개인 메서드
	double sum ( int a, double b) {
		
		return a + b;
	}
	//4. 리턴타입 없음(void), 내부 함수를 종료(return;)
		//메서드 내부에서 return;을 만나면 메서드 종료
	void printMethod (int m) {
		if (m < 0 || m > 12) {
			System.out.println("잘못된 입력, 메서드 종료");
			return;		//메서드 종료
...
public class Using_Method01 {
	...
	public static void main(String[] args) {
		...
		System.out.println("=====리턴이 없고 잘못된 입력이 되면 메서드 종료=====");
		b.printMethod(13);		//1~12만 입력 가능, 그 외에는 메서드 종료
