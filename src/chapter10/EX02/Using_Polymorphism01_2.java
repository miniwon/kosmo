...
public class Using_Polymorphism01_2 {
	...
	public static void main(String[] args) {
		...
		//		- Animal 타입의 필드, 메서드만 접근 가능, Person 타입으로 다운캐스팅 가능
		Animal a2 = new Person();
		System.out.println(a2.name);
		
			// a2는 Person 타입으로 다운캐스팅(Animal, Person 필드와 메서드 모두 접근 가능)
		Person p1 = (Person) a2;
		System.out.println(p1.name);
		System.out.println(p1.personName);
		
		// 3. Person 객체를 Person 타입으로 생성(Animal, Person 필드와 메서드 모두 접근 가능)
		Person p2 = new Person();
		System.out.println(p2.name);
		System.out.println(p2.personName);
			// 업캐스팅
		Animal a3 = p2;		// a3는 Animal, Person 내포하고 있고, Animal 타입으로 지정
		...
		// 4. Person_person 객체를 Animal 타입으로 생성
			// a4는 Animal, Person, Person_person 타입을 내포, Animal 타입으로 지정
		...
		// 5. Person_person 객체를 Person_person 타입으로 생성
			// pp2는 Animal, Person, Person_perosn을 내포, Person_perosn 타입으로 지정
