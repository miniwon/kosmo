...
class Cc {					// 부모 클래스
//	Cc(){}			// 객체 내부에 생성자가 존재할 경우 컴파일러는 기본 생성자를 자동으로 추가하지 않는다
	Cc (int a) {
		System.out.println("Cc 입력매개변수 1인 생성자 - Cc(int a)");
	}
}
class Dd extends Cc {		// 자식 클래스
	/*
	 * Dd 클래스의 생성자가 존재하지 않을 때 컴파일러는 기본 생성자를 자동으로 추가한다
	 * Dd(){
	 * 		super();	<- 기본적으로 생략
	 * }
	 */
	...
	Dd (int a) {
		this();			// 자신의 객체의 생성자 호출
		System.out.println("Dd의 입력매개변수 1개인 생성자 호출");
	}
}
